// Regression test: runs the manifest's content scripts in jsdom against real
// Teams DOM fixtures. npm install once, then npm test
const { JSDOM } = require('jsdom');
const fs = require('fs');
const path = require('path');

const manifest = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'manifest.json'), 'utf8'));
const SOURCES = manifest.content_scripts[0].js.map((p) =>
  fs.readFileSync(path.join(__dirname, '..', p), 'utf8')
);

// loads exactly what the manifest ships, in the same order
const loadWindow = (bodyHtml, { platform, seen = true } = {}) => {
  const dom = new JSDOM(`<!doctype html><html><body>${bodyHtml}</body></html>`, {
    url: 'https://teams.microsoft.com/v2/',
    runScripts: 'outside-only',
    pretendToBeVisual: true,
  });
  if (platform) Object.defineProperty(dom.window.navigator, 'platform', { value: platform });
  Object.defineProperty(dom.window.navigator, 'clipboard', {
    value: { writeText: async (t) => { dom.window.__copied = t; } },
  });
  if (seen) dom.window.localStorage.setItem('ttc-seen', 'true');
...
const FIXTURES = rileyMsg + jordanMsg + edgeMsg + attachMsg;

(async () => {
  const win = loadWindow(FIXTURES);
  const ttc = win.__ttc;
  ...
  const win2 = loadWindow(pane);
  const ttc2 = win2.__ttc;
  ...
  const macWin = loadWindow(FIXTURES, { platform: 'MacIntel' });
  const mac = macWin.__ttc;
  const macIds = [...mac.store.keys()].sort();
  const macNode = (id) => macWin.document.getElementById(`message-body-${id}`);
  const click = (id, mods) =>
    macNode(id).dispatchEvent(new macWin.MouseEvent('click', { bubbles: true, cancelable: true, ...mods }));
  ...
  // ---- panel expansion + alt+c shortcut + copy prefs ----
  const tick = () => new Promise((r) => setTimeout(r, 5));
  const altC = (w) =>
    w.document.dispatchEvent(new w.KeyboardEvent('keydown', { code: 'KeyC', altKey: true, bubbles: true, cancelable: true }));

  const firstWin = loadWindow(FIXTURES, { seen: false });
  const uxWin = loadWindow(FIXTURES);
  const ux = uxWin.__ttc;
  const uxIds = [...ux.store.keys()].sort();

  const idlePill = !ux.isExpanded();
  ux.toggle(uxIds[0]);
  ux.toggle(uxIds[1]);
  const autoExpanded = ux.isExpanded();

  altC(uxWin);
  await tick();
  const copiedDefault = uxWin.__copied || '';
  const keptByDefault = JSON.parse(ux.json()).length === 2 && ux.isExpanded();

  ux.prefs.set('clearAfterCopy', true);
  ux.prefs.set('collapseAfterCopy', true);
  altC(uxWin);
  await tick();
  const optInsApplied = JSON.parse(ux.json()).length === 0 && !ux.isExpanded();

  const input = uxWin.document.createElement('input');
  uxWin.document.body.appendChild(input);
  input.focus();
  uxWin.__copied = null;
  ux.toggle(uxIds[0]);
  altC(uxWin);
  ...
  const uxChecks = {
    ...
    'alt+c copies the selection as markdown': copiedDefault.includes('[attachment:') || copiedDefault.includes('> replying to'),
    'alt+c keeps selection and panel by default': keptByDefault,
    'alt+c honors the clear and collapse opt-ins': optInsApplied,
    'alt+c is ignored while typing': ignoredWhileTyping,
  };
  ...
  const total = [quoteChecks, paneChecks, selectionChecks, imageChecks, gestureChecks, uxChecks, dragChecks]
